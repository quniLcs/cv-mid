...
def drawAvgLoss(loss_log_path):
    ...
    """
    line_cnt = 0
    for count, line in enumerate(open(loss_log_path, "r")):
        line_cnt += 1
    ...
    for i in range(len(result["avg"].values)):
        writer.add_scalars('loss', {'train': float(result["avg"].values[i])}, i+1)
    writer.flush()
    writer.close()
